...
class Ship:
    ...
    def fire(self):
        ...
        missile_velocity = [self.vel[0] + 5 * forward[0], self.vel[1] + 5 * forward[1]]
        a_missile = Sprite(missile_position, missile_velocity, self.angle, 0, missile_image, missile_info,
                           missile_sound)
...
def rock_spawner():
    ...
    a_rock = Sprite([random.randrange(WIDTH), random.randrange(HEIGHT)],
                    [random.choice([0.5, -1]) * random.random() * 2, random.choice([0.5, -1]) * random.random() * 2], 1,
                    random.choice([0.5, -1]) * 0.05, asteroid_image, asteroid_info)
...
# http://www.codeskulptor.org/#user38_SPnXfO2nET_0.py
